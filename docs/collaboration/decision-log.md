...
## DEC-013 — v3 typed model states and decision priority

Date: 2026-08-05 · Owner: Codex · Reviewer: Claude Opus 5 (`claude-opus-5`, high)

- Decision: ratio and capacity outcomes retain typed valid, missing, invalid,
  not-meaningful, blocked, and policy-not-applicable states through scoring, policy,
  API, and interface layers. Critical blocked measures prevent a final grade.
- Decision priority: no supportable exposure → decline; hard policy stop or blocked
  critical analysis → decline; otherwise capacity, stress, and conditions apply.
- Reverse stress uses a bounded bisection contract with explicit non-convergence
  metadata. A label without solver evidence is not accepted as reverse stress.

## DEC-014 — v3 anonymous operations and persistence truthfulness

Date: 2026-08-05 · Owner: Codex · Reviewer: Claude Opus 5 (`claude-opus-5`, high)

- Public sessions are generated by the server as cryptographic HttpOnly cookies and
  stored as hashes. Client-selected session headers are accepted only in Pytest or
  when an explicit local-test environment flag is enabled.
- PostgreSQL is the durable target. If `DATABASE_URL` is absent, the product exposes
  temporary-session mode in `/runtime` and the case-list UI rather than promising
  persistence.
- No Supabase project is created without explicit authorization because project
  creation can introduce external resources and billing.
